...
class CastomSerializer(ModelSerializer):
    ...
    def validate_string_length(self, field_name, value):
        if len(value) > 50:
            raise ValidationError(
                f'Некорректный размер поля {field_name}. '
                'Размер должен быть не больше 50 символов.'
            )
        return value
    ...
    def validate_type(self, field_name, value, required_type):
        if not isinstance(value, required_type):
            raise ValidationError(
                f'Значение поля {field_name} должно быть типа '
    ...
    def validate_date(self, value):
        try:
            ...
            raise ValidationError(
                'Значение поля date должно быть в формате YYYY-MM-DD.'
            )
        return value
...
class StoresSerializer(CastomSerializer):
    ...
    def validate_st_id(self, value):
        ...
        if len(value) > 50:
            raise ValidationError(
                'Некорректный id магазина.',
                'Размер должен быть не больше 50 символов.'
            )
        if self.Meta.model.objects.filter(st_id=value).exists():
            raise ValidationError(
                'Уже существует магазин с таким id.'
            )
        return value
...
class CategoriesSerializer(CastomSerializer):
    ...
    def validate_pr_sku_id(self, value):
        ...
        if self.Meta.model.objects.filter(st_id=value).exists():
            raise ValidationError(
                'Уже существует товар с таким id.'
            )
        return value
...
class SalesForecastSerializer(CastomSerializer):
    ...
    def validate_target(self, value):
        if not isinstance(value, int):
            raise ValidationError(
                'Значение поля спрос должно быть типа int.'
            )
        if value < 0:
            raise ValidationError(
                'Значение поля спрос должно быть больше ноля.'
            )
        return value
